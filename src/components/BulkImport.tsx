...
function resolveNpmAlias(version: string): string | null {
...
function parsePackageJson(raw: string): string[] {
  ...
  for (const field of depFields) {
    ...
    if (section && typeof section === 'object' && !Array.isArray(section)) {
      for (const [alias, version] of Object.entries(section as Record<string, unknown>)) {
        // Resolve npm aliases: "foo": "npm:real-pkg@^1" → track "real-pkg"
        const resolved = typeof version === 'string' ? resolveNpmAlias(version) : null;
        names.add(resolved ?? alias);
      }
    }
  }
